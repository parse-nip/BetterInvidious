...
# React SPA handler: serves React app for UI routes when assets/react/index.html exists
# Must be added before register_all so it runs before route handlers
if File.file?(File.join("assets", "react", "index.html"))
  add_handler Invidious::HttpServer::ReactSpaHandler.new
end
